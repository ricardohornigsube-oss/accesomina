...
test('tenant integration secrets are encrypted and authenticated',()=>{const value={token:'private-token',pass:'private-password'},encrypted=encryptJson(value),tampered=`${encrypted.slice(0,28)}${encrypted[28]==='A'?'B':'A'}${encrypted.slice(29)}`;assert.equal(encrypted.includes(value.token),false);assert.deepEqual(decryptJson(encrypted),value);assert.throws(()=>decryptJson(tampered));});
